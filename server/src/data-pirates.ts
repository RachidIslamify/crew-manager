// ===========================================================================
//  data-pirates.ts — backend-kopie van de roster (was crew-manager/data-pirates.js)
//  Zelfde data als de frontend; alleen de browser-regels (window.*) zijn
//  weggelaten en de export is TypeScript-native. De backend gebruikt PIRATES.
//
//  LET OP: dit is nu een APARTE kopie van crew-manager/data-pirates.js.
//  Pas je later de roster aan, werk dan BEIDE bestanden bij.
// ===========================================================================

export interface Pirate {
  n: string;            // naam
  r: string;            // primaire rol
  img?: string;         // foto-bestand
  alt?: string[];       // extra rollen
  cap?: boolean;        // ook kiesbaar als captain
  navy?: boolean;       // vijand-only (marine)
  p: number;            // power
  d: number;            // defense
  s: number;            // speed
  c: string;            // start-crew ("Free Agent" = vrij op de markt)
  sp?: string[];        // special attacks
}
...
// --- Special-attack effecten ---------------------------------------------
export const SP_FX: Record<string, string> = {
